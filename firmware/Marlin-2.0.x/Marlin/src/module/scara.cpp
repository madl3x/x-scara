...
void forward_kinematics_SCARA(const float &a, const float &b) {
   ...
   */
    ...
    cartes.set(a, b-a/ELBOW_REDUCTION_RATIO);
  }
...
void inverse_kinematics(const xyz_pos_t &raw) {
  ...
  /* X_SCARA_COORDINATES_RELATIVE_ANGLES - X,Y are now angles */
    ...
    delta.set(raw.x, raw.y + raw.x/ELBOW_REDUCTION_RATIO, raw.z);
...
static  FORCE_INLINE void x_scara_fwdk(const float &a, const float &b) {
    const float S = a;
    const float E = b-a/ELBOW_REDUCTION_RATIO;
...
static FORCE_INLINE void x_scara_invk(const xyz_pos_t &raw) {
    ...
    float E =              ACOS((x2y2 - PK_SumLenSq   )/(PK_ProdOfLen)) + S/ELBOW_REDUCTION_RATIO;
